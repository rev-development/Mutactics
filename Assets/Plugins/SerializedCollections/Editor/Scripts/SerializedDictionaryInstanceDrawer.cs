    ...
    public class SerializedDictionaryInstanceDrawer
    {
        ...
        public SerializedDictionaryInstanceDrawer(SerializedProperty property, FieldInfo fieldInfo) {
            ...
            var listField = _fieldInfo.FieldType.GetField
                (SerializedDictionaryDrawer.SerializedListName, BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        public float GetPropertyHeight(GUIContent label) {
            if (!ListProperty.isExpanded) return SerializedDictionaryDrawer.TopHeaderClipHeight;
        ...
        private void CheckIfNewDictionary() {
            ...
            {
                var dictionary = SCEditorUtility.GetPropertyValue
                    (ListProperty, ListProperty.serializedObject.targetObject);
        ...
        private void UpdatePaging() {
            ...
            for (var i = startIndex; i < endIndex; i++) _pagedIndices.Add(i);
        ...
        private void ToggleSearchBar(bool flag) {
            ...
            if (!_showSearchBar)
            {
                if (_searchField.HasFocus()) GUI.FocusControl(null);
        ...
        private void DoKeyValueRect(Rect rect) {
            ...
            if (_propertyData != null)
            {
                if (Event.current.type == EventType.Repaint)
                {
                    _keyValueStyle.Draw
                        (
                            ...
                            EditorGUIUtility.TrTextContent
                                (_propertyData.GetElementData(SerializedDictionaryDrawer.KeyFlag).Settings.DisplayName),
                            false,
                            false,
                            false,
                            false
                    ...
                    _keyValueStyle.Draw
                        (
                            ...
                            EditorGUIUtility.TrTextContent
                                (
                                    _propertyData.GetElementData
                                                  (SerializedDictionaryDrawer.ValueFlag)
                ...
                var newWidth = SCEditorUtility.DoHorizontalScale
                    (changeSizeRect, _propertyData.KeyLabelWidth > 0f ? _propertyData.KeyLabelWidth : width);
        ...
        private float GetDesiredKeyLabelWidth(float maxWidth, float offset = 0f) {
            var desiredWidth = _propertyData is
            {
                KeyLabelWidth: > 0
            }
        ...
        private void ApplyPopulator(IEnumerable<object> elements, ModificationType modificationType) {
            foreach (var targetObject in ListProperty.serializedObject.targetObjects)
            {
                ...
                else if (modificationType == ModificationType.Confine) ConfineElements(lookupTable, elements);
        ...
        private static void AddElements(IKeyable lookupTable, IEnumerable<object> elements) {
            foreach (var key in elements)
            {
                ...
                if (occurences.Count > 0) continue;
        ...
        private static void ConfineElements(IKeyable lookupTable, IEnumerable<object> elements) {
            ...
            foreach (var key in elements) keysToRemove.Remove(key);
        ...
        private static void RemoveElements(IKeyable lookupTable, IEnumerable<object> elements) {
            var indicesToRemove = elements.SelectMany
                                           (x => lookupTable.GetOccurences(x))
        ...
        private void RemoveConflicts() {
            foreach (var targetObject in ListProperty.serializedObject.targetObjects)
            {
                ...
                {
                    var occurences = lookupTable.GetOccurences(key);
                    for (var i = 1; i < occurences.Count; i++) duplicateIndices.Add(occurences[i]);
                }
        ...
        private void OnDrawElement(Rect rect, int index, bool isActive, bool isFocused) {
            ...
            var keyRect = rect.WithSize
                (GetDesiredKeyLabelWidth(rect.width) - lineLeftSpace, EditorGUIUtility.singleLineHeight);

            var lineRect = keyRect.WithXAndWidth
                                   (keyRect.x + keyRect.width + lineLeftSpace, lineWidth)
