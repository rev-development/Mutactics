...
namespace AYellowpaper.SerializedCollections.Editor
{
    internal static class SCEditorUtility
    {
        ...
        public static float CalculateHeight(SerializedProperty property, bool drawAsList) {
            ...
            {
                float height = 0;
                foreach (var child in GetChildren(property)) height += EditorGUI.GetPropertyHeight(child, true);
        ...
        public static PropertyData GetPropertyData(SerializedProperty property) {
            ...
            if (json != null) EditorJsonUtility.FromJsonOverwrite(json, data);
        ...
        public static bool HasDrawerForProperty(SerializedProperty property, Type type) {
            var attributeUtilityType = typeof(SerializedProperty).Assembly.GetType
                ("UnityEditor.ScriptAttributeUtility");

            if (attributeUtilityType == null) return false;

            var getDrawerMethod = attributeUtilityType.GetMethod
                ("GetDrawerTypeForPropertyAndType", BindingFlags.Static | BindingFlags.NonPublic);

            if (getDrawerMethod == null) return false;
        ...
        internal static bool TryGetTypeFromProperty(SerializedProperty property, out Type type) {
            try
            {
                ...
                var methodInfo = classType.GetMethod
                    ("GetFieldInfoFromProperty", BindingFlags.Static | BindingFlags.NonPublic);
        ...
        internal static bool DoButtonControl(
            Rect rect,
            int id,
            bool on,
            bool hover,
